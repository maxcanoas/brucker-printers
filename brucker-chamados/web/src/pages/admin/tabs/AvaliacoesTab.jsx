...
export default function AvaliacoesTab({ tecnicos }) {
  ...
  const thStyle = { color: theme.textSecondary, fontSize: '12px', textAlign: 'left', padding: '8px 12px', borderBottom: `1px solid ${theme.border}`, backgroundColor: theme.bg };
  const tdStyle = { color: theme.text, fontSize: '14px', padding: '10px 12px', borderBottom: `1px solid ${theme.border}` };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <div style={cardStyle}>
          <p style={{ color: theme.textSecondary, fontSize: '13px', margin: '0 0 8px' }}>Nota Média</p>
        ...
        <div style={cardStyle}>
          <p style={{ color: theme.textSecondary, fontSize: '13px', margin: '0 0 8px' }}>Total de Avaliações</p>
          <span style={{ color: theme.text, fontSize: '32px', fontWeight: 700 }}>{stats.total}</span>
        </div>
        <div style={cardStyle}>
          <p style={{ color: theme.textSecondary, fontSize: '13px', margin: '0 0 8px' }}>Distribuição</p>
            ...
            {[5, 4, 3, 2, 1].map(n => {
              ...
              return (
                <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
                  <span style={{ color: theme.textSecondary, width: '15px' }}>{n}</span>
                  <Star size={10} fill="#C9A227" color="#C9A227" />
                  <div style={{ flex: 1, height: '8px', backgroundColor: theme.border, borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ width: `${pct}%`, height: '100%', backgroundColor: '#C9A227', borderRadius: '4px' }} />
                  </div>
                  <span style={{ color: theme.textSecondary, width: '25px', textAlign: 'right' }}>{item.count}</span>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ ...cardStyle, marginBottom: '24px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div>
            <label style={{ color: theme.textSecondary, fontSize: '13px', display: 'block', marginBottom: '6px' }}>Nota</label>
          ...
          <div>
            <label style={{ color: theme.textSecondary, fontSize: '13px', display: 'block', marginBottom: '6px' }}>Técnico</label>
      ...
      <div style={cardStyle}>
        {carregando && avaliacoes.length === 0 ? (
          ...
          <p style={{ color: theme.textSecondary, textAlign: 'center', padding: '40px' }}>Nenhuma avaliação encontrada</p>
        ) : (
          <>
                ...
                <tbody>
                  {avaliacoes.map((a, index) => (
                    <tr key={a.id} className="table-row" style={{ backgroundColor: index % 2 === 1 ? 'rgba(30, 37, 51, 0.5)' : 'transparent' }}>
                      ...
                      <td style={tdStyle}>{renderEstrelas(a.nota)} <span style={{ color: theme.textSecondary, fontSize: '12px', marginLeft: '4px' }}>{a.nota}/5</span></td>
                      <td style={{ ...tdStyle, maxWidth: '350px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                        {a.comentario || <span style={{ color: '#3A4553' }}>—</span>}
                      </td>
                      <td style={{ ...tdStyle, color: theme.textSecondary, fontSize: '13px' }}>
            ...
            {/* Paginação */}
            {totalPaginas > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginTop: '16px', paddingTop: '16px', borderTop: `1px solid ${theme.border}` }}>
                <button
                  onClick={() => buscar(pagina - 1)}
                  disabled={pagina <= 1}
                  className="btn-secondary"
                  style={{ ...btnPrimary, backgroundColor: 'transparent', border: `1px solid ${theme.border}`, color: theme.textSecondary, opacity: pagina <= 1 ? 0.5 : 1 }}
                >
                  Anterior
                </button>
                <span style={{ color: theme.textSecondary, fontSize: '14px', display: 'flex', alignItems: 'center' }}>
                  {pagina} / {totalPaginas}
                </span>
                <button
                  onClick={() => buscar(pagina + 1)}
                  ...
                  className="btn-secondary"
                  style={{ ...btnPrimary, backgroundColor: 'transparent', border: `1px solid ${theme.border}`, color: theme.textSecondary, opacity: pagina >= totalPaginas ? 0.5 : 1 }}
                >
                  Próxima
                </button>
              </div>
